	...
	*/
	ISR(TIM0_COMPA_vect) {
	  ...
	  if (timestamp % ADC_SAMPLE_RATE == 0 && latestADCSampleChecked) {
		  ...
		  latestADCSample = read_current_adc_value();
	...
	int main(void) {
		...
		while (1) {
	  		  if (!latestADCSampleChecked && calibrationState != 2){
				   uint8_t currentADCButton = get_current_adc_button(latestADCSample);
				   update_button_states(buttonCounts,debouncedCounts, buttonDebounced,buttonTapped,buttonHeld, currentADCButton);
